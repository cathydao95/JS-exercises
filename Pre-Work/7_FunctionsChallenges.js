...
function removeFromString(str, index, num){
    let newStr = str.slice(0, index) + str.slice(index + num);
    return newStr;
}
...
// The collection can be a string, an array, or an object. If the collection is a string or array, the third parameter is a starting index for where to search from.
...
function includes(collection, value, starting){
    ...
    if (starting === undefined){
        return collection.includes(value);
            ...
            for (let i=starting; i< collection.length; i++){
